...
int main(void)
{
            ...
            struct // struttura per un item
            {
                ...
                union // unione anonima per i negozi
                {
                    ...
                    struct // struttura per un computer
                    {
                        ...
                        bool printer;
                        bool scanner;
                        bool integrated_video_card;
                    } computer;
                };
            } item[NR_ITEMS]; // assumiamo per semplicità che vi siano solo 10 item per  
                              // negozio... dimensione totale 1160 byte
        } store[NR_STORE]; // assumiamo per semplicità che vi siano solo 3 store...
    };
    ...
    strcpy(sc.store[BOOK_STORE].item[0].book.title, "Programmare in Java");
    strcpy(sc.store[BOOK_STORE].item[0].book.author, "Pellegrino Principe");
    sc.store[BOOK_STORE].item[0].book.number_of_pages = 832;
